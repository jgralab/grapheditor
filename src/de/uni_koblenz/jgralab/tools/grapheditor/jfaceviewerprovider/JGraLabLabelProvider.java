 ...
 *
 */
public class JGraLabLabelProvider extends LabelProvider implements IEntityStyleProvider  {
	...
	public String getText(Object element){
		if(element instanceof Vertex){
			...
			String label = "v"+vertex.getId()+": "+vertex.getAttributedElementClass().getQualifiedName();
			PVector<Attribute> attributes = (PVector<Attribute>) vertex.getAttributedElementClass().getAttributeList();
			for(Attribute attr : attributes){
				if(vertex.getAttribute(attr.getName())!=null){
					label += "\n"+ attr.getName()+": "+ this.getStringRepresentation(vertex.getAttribute(attr.getName()));//vertex.getAttribute(attr.getName()).toString();
				}
			}
			return label;
		}else if(element instanceof Edge){
			Edge edge = (Edge) element;
			String label = "e"+edge.getId()+": "+edge.getAttributedElementClass().getQualifiedName();
			PVector<Attribute> attributes = (PVector<Attribute>) edge.getAttributedElementClass().getAttributeList();
			for(Attribute attr : attributes){
				if(edge.getAttribute(attr.getName())!=null){
					label += "\n"+attr.getName()+": "+ this.getStringRepresentation(edge.getAttribute(attr.getName()));// edge.getAttribute(attr.getName()).toString();
				}
			}
			return label;
